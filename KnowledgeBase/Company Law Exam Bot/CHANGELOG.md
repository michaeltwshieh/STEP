...
`Course-Manual-Self-Assessment-Answers.md` was dropped from this folder, and its entries removed from `CLAUDE.md`'s source-of-truth inventory and from `README.md`. It is not useful under exam conditions. The file still exists in `../STEP Advanced Certificate in Company Law and Practice/` and is regenerated by `split_corpus.py`, so this is reversible — do not re-copy it into this folder when re-syncing content.

Note: the July 2026 entry below mentions Self-Assessment Answers as part of what that revision added. That is left as written, being a record of what happened at the time rather than a live instruction.

## 2026-08-16 — folder renamed and completed

Renamed from `Proposed-Revisions-v2/`. Gained `Syllabus.md` (which its `CLAUDE.md` had always cited but never actually contained), both exam-admin documents, and a proper folder-guide `README.md`; the previous README became this changelog.

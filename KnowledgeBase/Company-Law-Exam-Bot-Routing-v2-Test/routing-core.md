...
Any exact entity/director count is a separate entity assertion and needs its own supplied
fact; do not infer a provider subsidiary's board composition from its corporate role.
...
1. A corporate member or corporate director is a separate legal actor. Derive and,
   where drafting is requested, produce its own upstream board authority and human-
   signatory instrument before the resolution it signs for the target company. A
   reference to an unspecified “authorised signatory” does not supply that instrument.
2. Derive the instrument count independently from every actor, approval threshold,
   transaction stage and requested deliverable. Do not let one Company resolution
   absorb an upstream corporate actor's decision.
3. For a notice that proposes transaction resolutions, the notice is incomplete unless
   it sets out every applicable core operative resolution required to conduct the
   stated business. A prior board decision to convene is `necessary companion/check`
   unless the question also asks to draft it.
   Derive the operative count from the action-specific precedent, not from the broad
   label in the question. One umbrella resolution (for example, only commencing the
   transaction) never silently discharges separately stated appointment, authority,
   remuneration, audit or completion resolutions. Include every operative resolution
   compatible with the supplied facts. If an individual operative depends on a missing
   fact, retain it as an expressly bracketed/conditional item or disclose the omission
   in the route dossier; never reduce the expected count without a source-based reason.
4. For a multi-document transaction, define and attach each distinct agreement,
   conveyance/bill of sale, facility, security, registry instrument and other exhibit
   supported by the facts. Do not hide an independently required exhibit inside a
   generic collective noun.
5. Select the closest precedent by transaction complexity and component architecture.
   A simpler precedent is forbidden once a complex selected precedent uniquely supplies
   required definitions, attachments, execution or amendment machinery.
...
execution blocks and records/filings. For a complex transaction, separately identify a
conveyance/Bill of Sale, facility, security and registry instrument and produce the
register-of-charges stage; a direction to update it later is not an actual entry.

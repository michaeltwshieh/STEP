...
### 12.3 Instrument-scope re-derivation

Derive the expected component list a second time without looking at the draft or its
first ledger. Re-open:

1. the question's complete purpose and every linked act needed to carry it out;
2. the actual governing articles;
3. the full action-specific appendix, including secondary resolutions; and
4. every companion precedent reached by the lifecycle pass.

Use that independent list as the final `expected` count. The draft's existing contents
must never define what was expected.

Apply these scope rules:

- A notice must reproduce the complete proposed business required to carry out its
  stated purpose. If the action-specific appendix contains fact-triggered resolutions
  appointing an office-holder, approving remuneration/cost/audit treatment or granting
  implementation powers, include them in the proposed-resolution text unless a higher
  source or the question excludes them. The notice heading alone does not narrow the
  business to the first resolution.
- If the requested output is "appropriate resolutions" and a registered member or
  director is itself a corporation, draft a separate internal authority resolution for
  the human signatory of each corporate actor unless supplied authority already exists.
  Merely naming an authorised signatory or listing the authority in the dossier is not
  a drafted authority instrument.
- If member approval is used for a linked acquisition/finance/security transaction,
  the member instrument must operatively approve every linked act that the member is
  asked or required to approve. A recital describing the acquisition, followed by an
  operative clause sanctioning only borrowing, is incomplete.
- Mark every incorporated transaction document as an exhibit or attachment in the
  relevant recital and approval clause. If the document is absent, use a stable exhibit
  placeholder and state the attachment gap; do not silently downgrade it to a mere
  definition.
- For an analogous complex-transaction precedent, select the precedent whose
  document/attachment/execution architecture matches the lifecycle. Treat a simpler
  precedent only as a cross-check when it omits stages that the facts trigger.

Before release, print an `Independent scope count` and an `Actual draft count`. If the
counts or component identities differ, the draft fails even when its own first ledger
said they matched.
...
- **contribution orphan:** a mandatory source's unique command/fact contribution is
  absent from the answer even though adjacent doctrine is present;
...
- **self-referential reconciliation:** expected components were derived from the draft
  or its first ledger instead of the independently re-opened action sources;
...
### 14.1 Mandatory Route-to-Answer Assertion Ledger

For every `must open` route, record the source's unique contribution and the exact
place where that contribution appears in the proposed answer. Do this before the prose
or draft is finalized.

| Sub-part/command dimension | Mandatory source | Unique contribution | Triggering fact or words | Required answer assertion/component | Present? |
|---|---|---|---|---|---|
| exact dimension | exact section/appendix | contribution unavailable from other selected sources | fact/command | sentence, calculation or operative component | yes/no |

Rules:

1. Split compound commands into dimensions. `Describe and explain why`, `compare and
   advise`, `identify risks and state how exposure is reduced`, and similar wording
   each require a separate assertion for every dimension.
2. Historical or causal commands require the complete causal contribution of the
   routed historical source, not an uncited general paraphrase from a neighbouring
   governance module.
3. A mitigation command includes both protective documents and operational conduct.
   If a routed lifecycle source supplies alternative operational responses to an
   identified risk state, the answer must state each supported alternative and its
   condition.
4. A source named in a dossier, file-open ledger or cross-check does not count unless
   its unique contribution is visible in the answer or requested instrument.
5. A plausible statement produced from general knowledge does not count as route
   recall if the source that uniquely supports the required dimension was not selected
   and its contribution was not mapped.

Run a removal test for each mandatory route: if removing the source would leave the
answer unchanged, either the source fails the hard relevance gate or its unique
contribution is missing. Discard the irrelevant source or add the missing supported
assertion. A mandatory route may not end with `Present? = no`.
...
11. Independent instrument-scope count and actual draft count.
12. Mandatory Route-to-Answer Assertion Ledger and removal test.
13. Materials gaps and source conflicts.
14. Mandatory critical-route checklist.
...
- the independently re-derived scope count matches the actual draft by component
  identity, not merely by number;
- every mandatory route has a visible unique contribution mapped to the answer;

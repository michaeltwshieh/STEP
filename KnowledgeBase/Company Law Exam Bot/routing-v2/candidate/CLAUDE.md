...
2. run `routing-core.md` and produce a pre-answer `RoutePlan` conforming to
   `routing-v2/schema/route-plan.schema.json`;
3. freeze its source allowlist, retrieve only those sources and complete its actual-open
   ledger, branch sets, document chain, gaps and final trace;
4. run `routing-v2/scripts/validate_route_plan.py` and retain its deterministic report;
5. only when the report says `VALID`, enter `section-a.md` for MCQs or `section-b.md`
   for prose/drafting.

No adapter or instruction file may copy a second legal mapping.
No adapter may render an answer from an invalid or absent RoutePlan, and completed prose
must never be parsed to reconstruct one.
...
shared core and the relevant adapter's done-when checklist both pass. The check trace
must also name the validated plan ID and canonical plan hash.

For blind and behavioral tests, run answer work through
`routing-v2/scripts/isolation_harness.py`. Gold, KAP, prior answers and peer output are
evaluator-only inputs created after every answer hash locks; an out-of-manifest request
is a hard isolation failure even if the file is not cited.

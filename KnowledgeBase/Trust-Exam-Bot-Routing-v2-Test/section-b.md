...
Run the single source map in `Content.md` and the shared algorithm in
`routing-core.md` first. This adapter contains only prose/drafting classification,
...
Treat every sub-part independently. Record its command word, marks, material facts,
requested deliverables and mode (`PROSE` or `DRAFTING`). Do not let the longest
sub-part classify the rest.

Before writing each sub-part, give all seven layers a verdict: incorporate,
checked-not-relevant, conditional or materials gap.

1. substantive rule;
2. validity, constitution and capacity;
3. power, decision-maker and procedure;
4. fiduciary duties, care and liability;
5. records, accounts, notices and registration;
6. documents; and
...
- trust type, transaction stage and regime;
- every operative part and selectable alternative;
- every incorporated schedule/attachment and the complete attachments set; and
- the complete execution block, seal, delivery, record and filing stages.

Consume the shared core's requested-document chain before drafting. State the expected
instrument/component/attachment counts privately, then reconcile them against the
produced draft. A corporate trustee resolution may itself authorise the trust act. A
distinct corporate settlor, protector, beneficiary or founder needs its own upstream
authority only where the facts and course materials support it; a generic signatory
...
An essay is more likely than an MCQ to span several modules, so **run the independent legal-relationship/lifecycle second pass in `routing-core.md` before finalising**. Incorporate a second source only if it supplies a unique rule, procedure, authority, document or consequence the question asks for. Record the result on the `Cross-checked:` line of the check panel, never in the submitted prose.
...
## Drafting sub-parts (the "draft" command, or a deed / trustees' resolution / memorandum / power of attorney / other instrument)

This is a **precedent-reproduction task**, not a prose task. The reword rule below is suspended for it.

1. **Open the named or implied Appendix first.** If the question says "draft a deed exercising clause X" or points to an Appendix, view that exact precedent file before drafting.
...
3. **Keep the selected precedent's operative wording**, including headings, recipient blocks, execution/date/signature blocks, and singular/plural and joint-and-several formulations (e.g. "the Trustees … jointly", "solely in its capacity as trustee of the Trust"). Do not paraphrase these into your own words. Fidelity applies only to a precedent selected for the requested instrument after the shared relevance and deliverable-scope gates. A precedent opened only to check a companion stage or reject a distractor is not reproduced.
...
**Doctrine must be paid for in particulars.** If a paragraph states a rule and moves on, it is exposed. Give the rule a figure, a named party, a jurisdictional variation, or an instrument clause or statutory section doing specific work — or cut the paragraph.
...
- **Use plain "is / are / has".** AI dresses up the verb: "serves as", "stands as", "operates as", "represents". A trustee *is* a fiduciary; it does not "serve as" one.
- **Cut "-ing" tails.** AI bolts a present participle onto the end to fake depth: "...the instrument is binding, reflecting the settlor's intention". Stop at the point; if the tail carries real content, make it its own sentence.
...
- **Repeat the precise term; do not cycle synonyms.** AI varies wording to avoid repetition ("the trust", then "the settlement", then "the arrangement"). Pick the correct term and reuse it. A "beneficiary" stays a "beneficiary".
- **Do not ask rhetorical questions.** Posing a question and answering it at once ("So what should a trustee do when directed?") reads as the machine organising itself in public. State it as a statement.
...
- [ ] **The particulars constraint holds everywhere.** Read the answer in 150-word chunks, including anything just added to close a coverage gap — prose written under time pressure to fill a box is the most exposed text in the answer. Every chunk contains either the question's own particulars (a name, figure, date, trust-instrument clause or statutory section doing specific work) or a dense source-specific enumeration. Any chunk that is general law in smooth prose gets rewritten or cut. This is the check that matters most in this sweep; run it before the others.
